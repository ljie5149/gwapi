<?php
    ...
    if ($method == 'GET') {
        ...
        if (json_last_error() === JSON_ERROR_NONE && is_array($delete_data)) {
            ...
            $src_data = $_GET; // 相容於把參數放 URL 查詢字串的 DELETE 請求
        }
    }
    ...
    $tableMain = 'default_device';
    $tableLog  = 'log_default_device';
        ...
        if ($conn_res["status"] == "true") {
            switch ($opt) {
                ...
                case "get":
                    ...
                    $sql = "SELECT id, sid, device_type, device_name, receive_mode, tag, 
                                   baudrate, data_bits, parity, stop_bit, handshake, hk, 
                    ...
                    $db->saveBusinessLog(
                        $link, $tableLog,
                        ['col_name' => 'device_type', 'val' => $device_type],
                        ['col_name' => 'device_name', 'val' => $device_name],
                        'GET', ['status' => 1, 'remark' => ''],
                        $member_id, $remote_ip, '取得預設設備型號設定資料'
                    );
                    break;
                ...
                case "insert":
                    ...
                    foreach ($items as $item) {
                        ...
                        // 驗證必填欄位 (sid, device_type, device_name)
                        if (empty($sid) || empty($device_type) || empty($device_name)) {
                            $has_error = true;
                            $processed_results[] = [
                                'sid'     => $sid,
                                'status'  => 'false',
                                'message' => '新增失敗，[sid]、[device_type] 與 [device_name] 為必填欄位！'
                            ];
                            continue;
                        }
                        ...
                        if ($chk_res && mysqli_num_rows($chk_res) > 0) {
                            ...
                            $update_sql = "UPDATE $tableMain 
                                           SET sid = ?, device_type = ?, device_name = ?, receive_mode = ?, 
                                               tag = ?, baudrate = ?, data_bits = ?, parity = ?, stop_bit = ?, 
                                               handshake = ?, hk = ?, status = ?, sort_order = ?, remark = ?, updated_at = NOW() 
                                           WHERE id = ?";
                            
                            $up_stmt = mysqli_prepare($link, $update_sql);
                            mysqli_stmt_bind_param($up_stmt, "sssssiissssiiii", 
                                $sid, $device_type, $device_name, $receive_mode, 
                                $tag, $baudrate, $data_bits, $parity, $stop_bit, 
                                $handshake, $hk, $status, $sort_order, $remark, $exist_id
                            );
                            ...
                            if ($exec_up && $affected_rows >= 0) {
                                ...
                                $db->saveBusinessLog(
                                    ...
                                    $member_id, $remote_ip, '更新預設設備資料成功'
                                );
                            } else {
                                $has_error = true;
                            ...
                            $insert_sql = "INSERT INTO $tableMain 
                                           (sid, device_type, device_name, receive_mode, tag, baudrate, data_bits, parity, stop_bit, handshake, hk, status, sort_order, remark, created_at) 
                                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                            
                            $in_stmt = mysqli_prepare($link, $insert_sql);
                            mysqli_stmt_bind_param($in_stmt, "sssssiissssiii", 
                                $sid, $device_type, $device_name, $receive_mode, 
                                $tag, $baudrate, $data_bits, $parity, $stop_bit, 
                                $handshake, $hk, $status, $sort_order, $remark
                            );
                            ...
                            if ($exec_in && mysqli_stmt_affected_rows($in_stmt) > 0) {
                                ...
                                $db->saveBusinessLog(
                                    ...
                                    $member_id, $remote_ip, '新增預設設備資料成功'
                                );
                            } else {
                ...
                case "edit":
                    ...
                    if ($exec_up && $affected_rows >= 0) {
                        ...
                        $db->saveBusinessLog(
                            $link, $tableLog,
                            ['col_name' => 'device_type', 'val' => $old_data['device_type']],
                            ['col_name' => 'device_name', 'val' => $old_data['device_name']],
                            'UPDATE', ['status' => 1, 'remark' => $who_call.' 呼叫 api '.$API_name],
                            $member_id, $remote_ip, '更新預設設備資料成功'
                        );
                    } else {
                ...
                case "delete":
                    ...
                    // 檢查目標是否存在
                    if ($id > 0) {
                        $chk_sql = "SELECT id, device_type, device_name FROM $tableMain WHERE id = ? LIMIT 1";
                        $chk_stmt = mysqli_prepare($link, $chk_sql);
                        mysqli_stmt_bind_param($chk_stmt, "i", $id);
                    } else if (!empty($sid)) {
                        $chk_sql = "SELECT id, device_type, device_name FROM $tableMain WHERE sid = ? LIMIT 1";
                        $chk_stmt = mysqli_prepare($link, $chk_sql);
                        mysqli_stmt_bind_param($chk_stmt, "s", $sid);
                    } else {
                        $chk_sql = "SELECT id, device_type, device_name FROM $tableMain WHERE device_type = ? LIMIT 1";
                        $chk_stmt = mysqli_prepare($link, $chk_sql);
                        mysqli_stmt_bind_param($chk_stmt, "s", $device_type);
                    }
                    ...
                    if ($exec_del && $affected_rows > 0) {
                        ...
                        $db->saveBusinessLog(
                            $link, $tableLog,
                            ['col_name' => 'device_type', 'val' => $target_data['device_type']],
                            ['col_name' => 'device_name', 'val' => $target_data['device_name']],
                            'DELETE', ['status' => 1, 'remark' => $who_call.' 呼叫 api '.$API_name],
                            $member_id, $remote_ip, '刪除預設設備資料成功'
                        );
                    } else {
                        $null_array["err"] = mysqli_error($link);
