<?php
    ...
    switch ($mode) {
        ...
        case "apply"        : $head_tag = 'GA'; $is_base64_image = true;  break;
    }
        ...
        if ($data["status"] == "true") {
                            ...
                            if (strEndWith($field, '_img')) {
                                ...
                                for ($j = 0; $j < count($image_info_base64); $j++) {
                                    ...
                                    if ($com_base64->field == $field) {
                                        if (!empty($com_base64->filename) && !empty($com_base64->base64)) {
                                            $s .= (strlen($s) > 0) ? ';;' : '';
                                            $s .= $com_base64->filename;
                                            $s .= (strlen($s) > 0) ? ';' : '';
                                            $s .= saveBase64PDF($com_base64->base64, $g_root_dir, $image_path, $uid.'_'.$n++
                                                                                        , $com_base64->filename,  $mode);
                                        }
                                    }
                                }
                                $dst_filename_array[$field] = $s;
                            }
                        }
                    }
                }
            ...
            for ($i = 0; $i < count($column_info); $i++) {
                ...
                if ($show) {
                    ...
                    if ($is_base64_image && 
                        (strEndWith($field, '_img') || strStartWith($field, 'pdf_'))
                        ) {
